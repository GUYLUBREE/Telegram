...
//api Bot
case 'quotes':
                result = await fetchJson(`https://docs-Botapi.herokuapp.com/api/randomquote?apikey=Syaa`)
                pk = `"${result.result.quotes}"\n- ${result.result.author}`
                await reply(pk)
                break
case 'quotesislami':
                result = await fetchJson(`https://docs-Botapi.herokuapp.com/api/randomquote/muslim?apikey=Syaa`)
...
case 'nuliskiri':
              jird = body.slice(11)
                if (args.length == 0) return await reply(`Example: ${prefix + command} KahfzXy`)
...
case 'glitch':
              jird = args.join(" ")
                ...
                break
case 'glitchtt':
                if (args.length == 0) return await reply(`Example: ${prefix + command} KahfzXy Bot`)
                reply("Tunggu Sebentar, Sedang Proses....")
...
case 'gameover':
  case 'grafity':
    case 'grafity2':
                if (args.length == 0) return await reply(`Example: ${prefix + command} KahfzXy KahfzXy`)
                reply("Tunggu Sebentar, Sedang Proses....")
